...
# ---------- SCENE ----------
class DoublePendulumHF_LF(Scene):
    def construct(self):
        ...
        tsL,th1L=sample(th1,dt_LF,sig_LF); tsL,th2L=sample(th2,dt_LF,sig_LF)
        ...
        mass1=Dot(radius=0.08,color=WHITE); mass2=Dot(radius=0.09,color=WHITE)
        hf1=Dot(radius=0.06,color=GREEN); lf1=Dot(radius=0.06,color=RED)
        hf2=Dot(radius=0.06,color=GREEN); lf2=Dot(radius=0.06,color=RED)
        self.add(rod1,rod2,mass1,mass2,hf1,lf1,hf2,lf2)
        ...
        # Axes for time series (right panel)
        ax = Axes(
        ...
        self.add(ax)
        title = Text("Joint Angles (HF vs LF)", font_size=26).next_to(ax, UP, buff=0.2)
        self.add(title)

        g1_H=VMobject(color=GREEN,stroke_width=3); g1_L=VMobject(color=RED,stroke_width=3)
        g2_H=VMobject(color=GREEN_E,stroke_width=3); g2_L=VMobject(color=RED_E,stroke_width=3)
        for g in (g1_H,g1_L,g2_H,g2_L): g.set_points_as_corners([ax.c2p(0,0)])
        self.add(g1_H,g1_L,g2_H,g2_L)

        # Helper
        def nearest(ts,tau): i=np.searchsorted(ts,tau)-1; return np.clip(i,0,len(ts)-1)
        window=4; fps=60; nF=int(T_anim*fps); dtF=1/fps
        frameT=np.linspace(0,T_anim,nF); idx_all=np.minimum((frameT/dt_truth).astype(int),len(t)-1)

        h1_hist,h2_hist,l1_hist,l2_hist=[],[],[],[]
        t_cur=0
        for tau in frameT:
            th1_k, th2_k = th1[idx_all[np.searchsorted(frameT, tau)]], th2[idx_all[np.searchsorted(frameT, tau)]]
            p1, p2 = tip_positions(th1_k, th2_k, L=L, origin=origin[:2])

            # True pendulum geometry
            ...
            # Sensor readings
            iH = np.searchsorted(tsH, tau) - 1
            iL = np.searchsorted(tsL, tau) - 1
            iH = np.clip(iH, 0, len(tsH) - 1)
            iL = np.clip(iL, 0, len(tsL) - 1)
            ...
            lf1.move_to([*p1L, 0]); lf2.move_to([*p2L, 0])

            # ---- Scrolling window logic ----
            x_min = max(0.0, tau - window)

            # Keep only recent data within the visible window
            maskH = tsH <= tau
            maskH &= tsH >= x_min
            maskL = tsL <= tau
            maskL &= tsL >= x_min
            ...
            l1_pts = [ax.c2p(tsL[j] - x_min, th1L[j]) for j in np.where(maskL)[0]]
            h2_pts = [ax.c2p(tsH[j] - x_min, th2H[j]) for j in np.where(maskH)[0]]
            l2_pts = [ax.c2p(tsL[j] - x_min, th2L[j]) for j in np.where(maskL)[0]]
            ...
            if len(l1_pts) > 1:
                g1_L.set_points_as_corners(l1_pts)
                g2_L.set_points_as_corners(l2_pts)
