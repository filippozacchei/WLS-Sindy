...
class eSINDy:
    ...
    def __init__(self, library_functions, features=[], feature_names=None, differentiation_method=None, smoother=None, smoother_kws=None):
        ...
        self.smoother = smoother if smoother is not None else (lambda x, **kwargs: x)        
    ...
    def simulate(self, x0, t, coef="median",
        ...
        # pick coefficients
        coef_matrix = (
            self.coef_median if coef == "median"
            else self.coef_mean if coef == "mean"
            else ValueError("coef must be 'median' or 'mean'")
        )

        # RHS
        def rhs(ti, xi):
            xi = xi.reshape(1, -1)
            Theta = self.library_functions.transform(xi)
            if self.col_norms_ is not None:
                Theta = Theta / self.col_norms_[None, :]
            dxdt = (Theta @ coef_matrix).ravel()
            if max_deriv is not None:
                dxdt = np.clip(dxdt, -max_deriv, max_deriv)
            return dxdt

        # integrate
        sol = solve_ivp(rhs, [t[0], t[-1]], x0,
                        t_eval=t, method=method,
                        rtol=rtol, atol=atol)

        if not sol.success:
            raise RuntimeError("ODE solver failed after fallback: " + str(sol.message))
